...
class UNetSD_temporal(nn.Module):
    ...
    def __init__(self, cfg, in_dim=7, dim=512, y_dim=512, context_dim=512, hist_dim=156, concat_dim=8, out_dim=6,
                 dim_mult=[1, 2, 3, 4], num_heads=None, head_dim=64, num_res_blocks=3, attn_scales=[1 / 2, 1 / 4, 1 / 8],
                 use_scale_shift_norm=True, dropout=0.1, temporal_attn_times=1, temporal_attention=True,
                 use_checkpoint=False, use_image_dataset=False, use_fps_condition=False, use_sim_mask=False,
                 misc_dropout=0.5, training=True, inpainting=True, video_compositions=['text', 'mask'],
                 p_all_zero=0.1, p_all_keep=0.1, zero_y=None, black_image_feature=None):
        ...
        self.num_heads = num_heads if num_heads else dim // 32
        self.head_dim = head_dim
        ...
        self.stc_proj = nn.Sequential(
            nn.Conv2d(64, 64, 3, padding=1),
            nn.SiLU(),
            nn.Conv2d(64, self.concat_dim, 3, padding=1)  # project to 8 channels
        )
        ...
        if temporal_attention and not USE_TEMPORAL_TRANSFORMER:
            ...
            self.time_rel_pos_bias = RelativePositionBias(heads=self.num_heads, max_distance=32)
        ...
        self.pre_image = nn.Sequential(nn.Conv2d(in_dim + concat_dim, in_dim, 1, padding=0))
        init_block = nn.ModuleList([nn.Conv2d(in_dim, dim, 3, padding=1)])

        if temporal_attention:
            init_block.append(TemporalAttentionMultiBlock(dim, self.num_heads, head_dim, rotary_emb=self.rotary_emb,
        ...
        for i, (in_dim, out_dim) in enumerate(zip(enc_dims[:-1], enc_dims[1:])):
            for j in range(num_res_blocks):
                block = nn.ModuleList([ResBlock(in_dim, self.embed_dim, dropout, out_channels=out_dim,
                                                use_scale_shift_norm=False, use_image_dataset=use_image_dataset)])
                if scale in attn_scales:
                    block.append(SpatialTransformer(out_dim, out_dim // head_dim, head_dim, depth=1,
                                                    context_dim=self.context_dim, disable_self_attn=False,
                                                    use_linear=True))
                    if temporal_attention:
                        block.append(TemporalAttentionMultiBlock(out_dim, self.num_heads, head_dim,
        ...
        self.middle_block = nn.ModuleList([
            ResBlock(out_dim, self.embed_dim, dropout, use_scale_shift_norm=False, use_image_dataset=use_image_dataset),
            SpatialTransformer(out_dim, out_dim // head_dim, head_dim, depth=1, context_dim=self.context_dim,
                               disable_self_attn=False, use_linear=True),
            TemporalAttentionMultiBlock(out_dim, self.num_heads, head_dim, rotary_emb=self.rotary_emb,
                                        use_image_dataset=use_image_dataset, use_sim_mask=use_sim_mask,
                                        temporal_attn_times=temporal_attn_times),
            ResBlock(out_dim, self.embed_dim, dropout, use_scale_shift_norm=False)
        ])
        ...
        for i, (in_dim, out_dim) in enumerate(zip(dec_dims[:-1], dec_dims[1:])):
            for j in range(num_res_blocks + 1):
                block = nn.ModuleList([
                    ResBlock(in_dim + shortcut_dims.pop(), self.embed_dim, dropout, out_dim,
                             use_scale_shift_norm=False, use_image_dataset=use_image_dataset)])
                if scale in attn_scales:
                    block.append(SpatialTransformer(out_dim, out_dim // head_dim, head_dim, depth=1,
                                                    context_dim=1024, disable_self_attn=False, use_linear=True))
                    if temporal_attention:
                        block.append(TemporalAttentionMultiBlock(out_dim, self.num_heads, head_dim,
                                                                 rotary_emb=self.rotary_emb,
                                                                 use_image_dataset=use_image_dataset,
                ...
                in_dim = out_dim
                if i != len(dim_mult) - 1 and j == num_res_blocks:
                    block.append(Upsample(out_dim, True, dims=2.0, out_channels=out_dim))
                    scale *= 2.0
    ...
    def forward(self, 
        ...
        device = x.device
        self.batch = batch
        assert stc_feat is not None, "stc_feat must be provided as input to UNetSD_temporal"

        concat = x.new_zeros(batch, self.concat_dim, f, h, w)
        stc = rearrange(stc_feat, 'b f c h w -> (b f) c h w')
        stc = self.stc_proj(stc)
        h_stc = stc.shape[2]
        stc = rearrange(stc, '(b f) c h w -> (b h w) f c', b=batch)
        stc = rearrange(stc, '(b h w) f c -> b c f h w', b=batch, h=h_stc)
        zero = torch.zeros(batch, dtype=torch.bool).to(device)
        keep = torch.zeros(batch, dtype=torch.bool).to(device)
        misc_dropout = partial(self.misc_dropout, zero=zero, keep=keep)
        if stc.shape[-2:] != (h, w):
            if stc.ndim == 5:
                b, c, f, h1, w1 = stc.shape
                stc = stc.reshape(b * f, c, h1, w1)
                stc = F.interpolate(stc, size=(h, w), mode='bilinear', align_corners=False)
                stc = stc.view(b, c, f, h, w)
            else:
                stc = F.interpolate(stc, size=(h, w), mode='bilinear', align_corners=False)

        concat = concat + misc_dropout(stc)
        ...
        if self.temporal_attention:
            if hasattr(self, "time_rel_pos_bias"):
...
# Keep the class name same for compatibility
UNetSTC = UNetSD_temporal
