...
# from kornia.color import RgbToYuv444, Yuv444ToRgb
import cv2
import numpy as np
import time
...
def PNG2YUV_sp(PNGFileName:str, SavePath:str):
    # PNG_img = 
    # cmd = strcat("ffmpeg -i ",strcat(inputPath,filename),".jpg -pix_fmt yuv444p ",strcat(outputPath,outputName,".yuv"));
    out_filename = SavePath + "/input_256x256_1.yuv"
    cmd = ['ffmpeg', '-y', '-i', PNGFileName, '-pix_fmt', 'yuv444p', out_filename, "-loglevel", 'quiet']
    # cmd = ['ffmpeg', '-hwaccel', 'cuda', '-hwaccel_device', '0', '-y', '-i', PNGFileName, '-pix_fmt', 'yuv444p', out_filename]#, "-loglevel", 'quiet']
...
def YUV2PNG_sp(YUVFileName:str, out_filename:str):
    ...
    # cmd = ['ffmpeg', '-f', 'rawvideo', '-framerate', '1', '-video_size', '640x510', '-pix_fmt', 'yuv444p', 'dst_range', '1', '-i', YUVFileName, '-pix_fmt', 'rgb24', "output.png"]
    cmd = ['ffmpeg', '-y','-f', 'image2', '-c:v', 'rawvideo',  '-framerate', '1', '-video_size', f'256x256', '-pix_fmt', 'yuv444p', '-i', YUVFileName, '-pix_fmt', 'rgb24', out_filename, "-loglevel", 'quiet']
    # cmd = ['ffmpeg', '-hwaccel', 'cuvid', '-hwaccel_output_format', 'cuda', '-y','-f', 'image2', '-c:v', 'rawvideo',  '-framerate', '1', '-video_size', f'256x256', '-pix_fmt', 'yuv444p', '-i', YUVFileName, '-pix_fmt', 'rgb24', out_filename, "-loglevel", 'quiet']
    # '-dst_range', '1',
    # print(YUVFileName)
    subprocess.call(cmd)  
...
def YUV2PNG(YUVFileName:str, out_filename:str):
    ...
    cmd = 'ffmpeg -y -f image2 -c:v rawvideo -framerate 1 -video_size 256x256 -pix_fmt yuv444p -i ' + YUVFileName + ' -pix_fmt rgb24' + out_filename + ' -loglevel quiet'
    os.system(cmd)
...
def Encoder_VVC_Single_sp(YUVFileName:str, SavePath:str):
    ...
    cmd = [EXECUTABLE_FILE_NAME, '-c', cfg_filename, '-i', YUVFileName, '-s', '256x256', '--InputBitDepth=8', '--InputChromaFormat=444', '-fr', str(frameRate), '--FramesToBeEncoded=1', '--QP=52', '-b', bin_filename, f'--ReconFile='+rec_filename, '--OutputBitDepth=8']
    # print(cmd)
...
def Encoder_VVC_Single(YUVFileName:str, SavePath:str):
    ...
    cmd = EXECUTABLE_FILE_NAME + ' -c ' + cfg_filename + ' -i ' + YUVFileName + ' -s 256x256 --InputBitDepth=8 --InputChromaFormat=444 -fr ' + str(frameRate) + ' --FramesToBeEncoded=1 --QP=52 -b '+ bin_filename + ' --ReconFile=' +rec_filename+ ' --OutputBitDepth=8' ' > /dev/null 2>&1'
    # print(cmd)
    os.system(cmd)
...
def main():
    ...
    start_time = time.time()
    Encoder_VVC_Single_sp ('/media/EXT0/daole/sd_scripts/VVC_Python/input_256x256_1.yuv', '/media/EXT0/daole/sd_scripts/VVC_Python')
    ...
    # YUV2PNG('/media/EXT0/daole/sd_scripts/VVC_Python/OrgUbuntu_encoder_intra_vtm_x0_yuv_QP52.yuv')
    ...
    # print(output[0].type())
    # save_yuv_444("out_test.yuv", output[0], output[1][:,0,:,:], output[1][:,1,:,:])
    # # yuv = rgb_to_yuv444 (img)
