...
def jpeg_encode(x, qf):
    ...
    x_luma = x_luma + x_luma.round().detach() - x_luma.detach()
    x_chroma = x_chroma + x_chroma.round().detach() - x_chroma.detach()
...
def jpeg_encode_partition_map(x, qf, partition_map):
    # Assume x is a batch of size (N x C x H x W)
    # [-1, 1] to [0, 255]
    x = (x + 1) / 2 * 255
    n_batch, _, n_size, _ = x.shape
...
def jpeg_decode_partition_map(x, qf, partition_map):
    # Assume x[0] is a batch of size (N x 1 x H x W) (luma)
    # Assume x[1:] is a batch of size (N x 2 x H/2 x W/2) (chroma)
    x_luma, x_chroma = x
    n_batch, _, n_size, _ = x_luma.shape
